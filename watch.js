...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (session?.user) {
      ...
      const profileRes = await supa.from('profiles').select('username').eq('id', currentUser.id).maybeSingle();
      currentProfile = profileRes.data || { username: currentUser.email.split('@')[0] };
    }
