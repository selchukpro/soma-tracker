...
function openPanel() {
  ...
  switchSpTab(currentSpTab || 'profile');
}
function closePanel() {
...
function switchSpTab(tab) {
  ...
  document.getElementById('sptab-story-btn').classList.toggle('active', tab==='story');
...
async function renderProfile() {
  ...
  const [workoutsRes, profileRes] = await Promise.all([
    supa.from('workouts').select('*').eq('user_id', currentUser.id)
  ...
  const total    = workouts.reduce((s, w) => s + w.reps, 0);
...
async function completeStoryEpisode(episode, repsAchieved) {
  ...
  if (!uid) { showToast('Not signed in — progress not saved.'); return; }
  ...
  }, { onConflict: 'user_id,chapter,episode' }).catch(e => console.error('story_progress:', e.message));
  ...
  await supa.from('workouts').insert({
    ...
    reps:     repsAchieved
  }).catch(e => console.error('workout:', e.message));
...
async function saveWorkout() {
  // Always get fresh session — avoids race condition with auth.js
  const { data: { session } } = await supa.auth.getSession();
  const uid = session?.user?.id;

  if (!uid) { showAuthModal(); return; }
  ...
  const { error } = await supa.from('workouts').insert({
    user_id: uid, exercise: mode, reps, duration_seconds: duration
  });
  ...
  // Refresh profile if panel is open
  if (currentSpTab === 'profile' && document.getElementById('side-panel').classList.contains('open')) {
    renderProfile();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const _bind = (id, fn) => { const el = document.getElementById(id); if(el) el.addEventListener('click', fn); else console.warn('SOMA: missing element #'+id); };
  _bind('open-panel-btn',    openPanel);
  _bind('close-panel-btn',   closePanel);
  _bind('sptab-profile-btn', () => switchSpTab('profile'));
  _bind('sptab-story-btn',   () => switchSpTab('story'));
  _bind('sptab-lb-btn',      () => switchSpTab('leaderboard'));
  _bind('save-workout-btn',  saveWorkout);
});
