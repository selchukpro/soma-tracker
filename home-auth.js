...
async function updateHero() {
  ...
  if (currentProfile) {
    ...
    // Load stats
    const { data: workouts } = await supa.from('workouts').select('reps').eq('user_id', currentUser.id);
    const { data: progress } = await supa.from('story_progress').select('chapter,episode').eq('user_id', currentUser.id);

    const totalReps = workouts?.reduce((s,w) => s+w.reps, 0) || 0;
    const sessions  = workouts?.length || 0;
    const totalXp   = (progress||[]).reduce((s,p) => s + (STORY_XP[`${p.chapter}-${p.episode}`]||0), 0);
    ...
    // Story badge
    const ep = (progress?.length || 0) + 1;
    if (ep <= 8) document.getElementById('story-badge').textContent = `Ch.1 · Ep.${ep} Next`;
    else document.getElementById('story-badge').textContent = '✅ Chapter 1 Complete';
...
function goToWatch()   { window.location.href = 'watch.html'; }
function goToStory()   {
