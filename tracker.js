...
async function startTracking() {
  ...
  try {
    ...
    // Auto-join stream room if ?room= param in URL
    const _roomParam = new URLSearchParams(window.location.search).get('room');
    if (_roomParam) {
      startStream(_roomParam).then(() => {
        updateStreamUI(true, _roomParam);
      });
    }
...
async function loop(ts) {
  ...
  if (showReps) {
    ...
    // Broadcast to stream room if active
    if (typeof streamBroadcast === 'function') streamBroadcast(kps, conf.score);
...
// ── STREAM UI ─────────────────────────────────
function updateStreamUI(active, roomId) {
  const btn = document.getElementById('btn-stream');
  if (!btn) return;
  if (active) {
    btn.classList.add('on');
    btn.title = 'Streaming to room ' + roomId;
  } else {
    btn.classList.remove('on');
    btn.title = 'Start Streaming';
  }
}

async function toggleStream() {
  if (typeof _streamActive === 'undefined') return;
  if (_streamActive) {
    stopStream();
    updateStreamUI(false, null);
  } else {
    // Prompt for room code or create new room
    const code = prompt('Enter room code to stream to (or leave blank to create a new room):');
    if (code === null) return; // cancelled
    let roomId = code.trim().toUpperCase();
    if (!roomId) {
      roomId = generateRoomId();
      await supa.from('rooms').insert({ id: roomId, name: 'SOMA Room', type: 'watch', created_by: currentUser?.id }).catch(()=>{});
    }
    await startStream(roomId);
    updateStreamUI(true, roomId);
    // Show share hint
    const toast = document.getElementById('toast');
    if (toast) { toast.textContent = 'Streaming! Room: ' + roomId; toast.classList.add('show'); setTimeout(()=>toast.classList.remove('show'), 4000); }
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const _streamBtn = document.getElementById('btn-stream');
  if (_streamBtn) _streamBtn.addEventListener('click', toggleStream);
