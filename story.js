...
async function reloadProgress() {
  ...
  // Load both story_progress AND profile in parallel
  const [progressRes, profileRes] = await Promise.all([
    supa.from('story_progress').select('chapter,episode,reps_done').eq('user_id', currentUser.id),
  ...
  // XP from story_progress (source of truth for episodes)
  let calcXp = 0;
  for(const ch of CHAPTERS)
    for(const ep of ch.episodes)
      if(completedSet.has(`${ch.id}-${ep.id}`)) calcXp += ep.xp;

  // Use whichever is higher — profile might have cached a higher value
  ...
  console.log(`Progress loaded: ${completedSet.size} episodes, calcXp=${calcXp}, profileXp=${profileXp}, showing=${xp}`);
}
...
async function saveEpisodeToServer(chapter, episode, repsAchieved, totalXp) {
  try {
    ...
    else      console.log('profile XP updated to', totalXp, '— next:', nextCh, nextEp);
