// ── story.js v3 — fixed: save, XP, next episode, abandon ──
...
let completedSet = new Set();   // "chapterId-episodeId" strings
let activeEp     = null;        // { chapter, episode } — kept until AFTER next ep starts
let lastCompleted = null;       // { chapter, episode, reps } — for "next" button
let repCount     = 0;
let repPhase     = 'up';
let repEma       = null;
let goalMet      = false;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  console.log('Story auth:', currentUser ? 'logged in as ' + currentUser.email : 'not logged in');
...
async function reloadProgress() {
  ...
  const {data, error} = await supa
    .from('story_progress')
    .select('chapter,episode,reps_done')
    .eq('user_id', currentUser.id);

  if (error) { console.error('Progress load error:', error.message); return; }
...
function renderChapters() {
  ...
  for (const ch of CHAPTERS) {
    ...
    ch.episodes.forEach((ep, i) => {
      ...
      const stateIcon = isDone?'✅':isLocked?'🔒':'▶';
...
async function startEpisode(chapter, episode) {
  ...
  document.getElementById('ep-progress-fill').style.background = 'var(--warn)';
...
function updateRepUI() {
  ...
  el.style.color='#fff'; setTimeout(()=>el.style.color='var(--accent)',250);
...
async function completeEpisode() {
  ...
  // Always use fresh session
  const { data: { session } } = await supa.auth.getSession().catch(()=>({data:{session:null}}));
  const uid = session?.user?.id;
  if (!uid) { alert('Please sign in to save progress.'); return; }
  ...
  // ── SAVE TO SUPABASE ──
  const { error: progressErr } = await supa.from('story_progress').upsert({
    user_id:      uid,
    chapter:      chapter.id,
    episode:      episode.id,
  ...
  if (progressErr) {
    console.error('story_progress save error:', progressErr.message);
    // Try plain insert as fallback
    await supa.from('story_progress').insert({
      user_id:   uid,
      chapter:   chapter.id,
      episode:   episode.id,
      reps_done: repsAchieved
    }).catch(e => console.error('Fallback insert error:', e.message));
  }
  ...
  await supa.from('workouts').insert({
    ...
    reps:     repsAchieved
  }).catch(e => console.error('Workout save error:', e.message));

  // Mark done locally IMMEDIATELY
  completedSet.add(`${chapter.id}-${episode.id}`);

  // Store for "next" button — keep reference AFTER activeEp cleared
  lastCompleted = { chapter, episode, reps: repsAchieved };
  activeEp      = null;

  // Recalculate XP from completedSet
  let totalXp = 0;
  for (const ch of CHAPTERS)
  ...
  document.getElementById('complete-xp').textContent      = '+' + episode.xp + ' XP';

  // Next button
  const nextEp  = chapter.episodes.find(e => e.id === episode.id + 1);
  const nextBtn = document.getElementById('complete-next-btn');
  if (nextEp) {
    nextBtn.textContent = `Next: Ep.${nextEp.id} →`;
    nextBtn.disabled    = false;
  } else {
    const nextCh = CHAPTERS.find(c => c.id === chapter.id + 1);
    if (nextCh) { nextBtn.textContent = `Start ${nextCh.title} →`; nextBtn.disabled = false; }
    else        { nextBtn.textContent = '🎉 Story Complete!';        nextBtn.disabled = true; }
  }
...
function goNextEpisode() {
  // Find next unlocked episode using updated completedSet
  for (const ch of CHAPTERS) {
    for (let i = 0; i < ch.episodes.length; i++) {
      const ep = ch.episodes[i];
      if (completedSet.has(`${ch.id}-${ep.id}`)) continue; // already done
      const prevDone = i === 0 || completedSet.has(`${ch.id}-${ch.episodes[i-1].id}`);
      if (prevDone) {
        startEpisode(ch, ep);
        return;
      }
...
function abandonEpisode() {
  ...
  goalMet   = false;
  showView('chapters');
  renderChapters(); // completedSet unchanged — progress preserved
}
