...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  _bind('complete-home-btn', () => { showView('chapters'); renderChapters(); });
...
async function reloadProgress() {
  ...
  // Profile XP (optional — 406 means no profile row yet)
  let profileXp = 0;
  const profileRes = await supa.from('profiles')
    .select('total_xp').eq('id', currentUser.id).maybeSingle();
  if (profileRes.data?.total_xp) profileXp = profileRes.data.total_xp;

  completedSet.clear();
  (progressRes.data||[]).forEach(p => completedSet.add(`${p.chapter}-${p.episode}`));

  // XP from story_progress (source of truth)
  let calcXp = 0;
  for(const ch of CHAPTERS)
    for(const ep of ch.episodes)
      if(completedSet.has(`${ch.id}-${ep.id}`)) calcXp += ep.xp;

  const xp = Math.max(calcXp, profileXp);
  document.getElementById('story-xp-pill').textContent = xp + ' XP';
  console.log(`Progress: ${completedSet.size} episodes done, xp=${xp}`);

  // If no profile row exists, create it now
  if (!profileRes.data) {
    console.log('No profile row found — creating...');
