...
async function doAuth() {
  ...
  try {
    if (authMode === 'signup') {
      if (!username) { errEl.textContent = 'Username is required.'; btn.textContent='Create Account'; btn.disabled=false; return; }
      const { data, error } = await supa.auth.signUp({ email, password });
      if (error) throw error;
      // Create profile row
      if (data.user) {
        const { error: profErr } = await supa.from('profiles').insert({ id: data.user.id, username });
        if (profErr) console.warn('Profile insert error:', profErr.message);
      }
...
async function loadProfile() {
  ...
  try {
    ...
    if (error || !data) {
      // Profile might not exist yet — use email prefix as fallback display
      currentProfile = { username: currentUser.email.split('@')[0], id: currentUser.id };
    } else {
      currentProfile = data;
    }
  } catch(e) {
...
function updateUserBtn() {
  ...
  if (currentProfile) {
    ...
    btn.style.borderColor = 'rgba(0,255,198,.4)';
    btn.style.background  = 'rgba(0,255,198,.08)';
  } else {
