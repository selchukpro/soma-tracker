...
function onPeerSkeleton(payload) {
  ...
  if (!uid) return;
  if (uid === me.id) return; // don't overwrite own data from broadcast echo
...
function renderLoop() {
  if (_renderInterval) clearInterval(_renderInterval);
  _renderInterval = setInterval(_renderFrame, 33); // ~30fps stable
}
...
function _renderFrame() {
  ...
  // Always keep canvas sized correctly
  const sw = stage.offsetWidth  || window.innerWidth;
  const sh = stage.offsetHeight || Math.round(window.innerHeight * 0.65);
  ...
  const now  = Date.now();
  // Show all peers — even those without kps yet (show placeholder)
  const allUids = Object.keys(peers);
  const activeUids = allUids.filter(uid => peers[uid].kps && (now - peers[uid].lastSeen) < 5000);
  ...
  if (!activeUids.length) {
    ...
    ctx.textAlign = 'left';
    updateScoreboard(allUids);
    return;
  }

  const cols  = Math.min(activeUids.length, 3);
  const rows  = Math.ceil(activeUids.length / cols);
  const cellW = canvas.width  / cols;
  const cellH = canvas.height / rows;

  activeUids.forEach((uid, i) => {
    const p   = peers[uid];
    const col = i % cols;
    ...
    const sx  = cellW / 640;
    const sy  = cellH / 480;
    const stale = Math.max(0.2, 1 - (now - p.lastSeen) / 4000);
